...
type (
	...
	AssetBillsDetails struct {
		Type   test.BillType `json:"type,string,omitempty"`
	...
	GetDepositHistory struct {
		...
		State  test.DepositState `json:"state,omitempty,string"`
	}
	Withdrawal struct {
		Ccy    string                     `json:"ccy"`
		...
		Dest   test.WithdrawalDestination `json:"dest,string"`
	}
	GetWithdrawalHistory struct {
		...
		Before int64                `json:"before,omitempty,string"`
		Limit  int64                `json:"limit,omitempty,string"`
		State  test.WithdrawalState `json:"state,omitempty,string"`
	}
